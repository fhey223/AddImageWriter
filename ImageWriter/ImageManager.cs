...
namespace ImageWriter
{
    ...
    public class ImageManager
    {
        ...
        public Bitmap SetTransparence(Bitmap bm, float transparence)
        {
            ...
            var bitmap = new Bitmap(bm.Width, bm.Height);
            var graphics = Graphics.FromImage(bitmap);
            graphics.DrawImage(bm, new Rectangle(0, 0, bm.Width, bm.Height), 0, 0, bm.Width, bm.Height,
        ...
        /// <param name="margin">水印边距: int</param>
        /// <param name="writepath">保存路径(包含完整路径,文件名及其扩展名): string</param>
        public void SaveWatermark(Bitmap sBitmap, Bitmap wBitmap, WatermarkPosition position, int margin,
            string writepath)
        {
        ...
        /// <param name="margin">水印边距: int</param>
        /// <param name="writepath">保存路径(包含完整路径,文件名及其扩展名): string</param>
        public void SaveWatermark(string readpath, string watermarkpath, float transparence, WatermarkPosition position,
            int margin, string writepath)
        {
            if (string.Compare(readpath, writepath) == 0)
                throw new ArgumentException("源图片与目标图片地址相同");
            if (transparence == 0.0f)
                ...
                SaveWatermark(new Bitmap(readpath), SetTransparence(watermarkpath, transparence), position, margin,
                    writepath);
        }
